...
/*   Updated: 2022/02/03 05:55:06 by gucamuze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	send_to_b(t_data *data, int c, int ret, int chunk_size)
{
	...
	{
		data->c_info.first_bot = get_first_bot_index(data->lst_arr, ft_lstsize(data->s_a), data->chunks[c], chunk_size);
...
void	sort_and_push(t_data *data, int chunk_size)
{
	...
	while (data->s_a)
	{
		// pretty_print(data->s_a, data->s_b);
		data->lst_arr = lst_to_array(data->s_a);
		if (chunk_is_empty(data->chunks[c], chunk_size))
			c++;
		data->c_info.first_top = get_first_top_index(data->lst_arr, ft_lstsize(data->s_a), data->chunks[c], chunk_size);
